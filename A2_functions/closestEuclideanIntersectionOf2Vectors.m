...
function [M,dM] = closestEuclideanIntersectionOf2Vectors(P1,v1,P2,v2)
%CLOSESTEUVLIDEANINTERSECTIONOF2VECTORS computes the approximate (symbolic) 
%intersection of two vectors, M, with an error metric, dM.
%[M,dM] = closestEuclideanIntersectionOf2Vectors(P1,v1,P2,v2) where vector 1 is
...
%defined by fixed point P2 and direction vector v2.
    ...
    if cross(v2,v1) == [0,0,0] %if the vectors are parallel
        ...
        L1 = P1+(t(1).*v1);
        L2 = P2+(t(2).*v2); 
        M = norm(L2+L1)/2; %Compute closest intersection
        dM = norm(L2-L1)/2; %Compute error in closest intersection
    end
